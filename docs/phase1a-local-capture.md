...
The subsequent v3 iPhone run restored pose analysis: 1,868 inference attempts succeeded with no analysis error, median inference duration was 23 ms, P95 was 26 ms, and one 2,296 ms maximum outlier was retained. The session contained 1,621 presented frames, 1,351 landmark-bearing pose results, 44,583 landmark rows, 1,351 signal rows, and eight detected trial records. File sizes matched the manifest, and all 1,621 selected source timestamps were unique, monotonic, and locked to `videoCurrentTime`. However, the 1,868 inferences linked to only 1,559 unique presented-frame IDs, proving that the camera loop sometimes analyzed one presented frame more than once. Build `phase1a-frame-dedup-v4` therefore permits at most one pose request for each recorded `presentedFrameIndex`. Trial-count validity and the Phase 1A short-run gate remain open until the deduplicated build is tested.

The first v4 run confirmed that deduplicated failure accounting works but also demonstrated that version-pinned CDN delivery remains unreliable: 1,251 frame timestamps were unique and monotonic, while the first pose request failed after 2,198 ms with `Load failed`; the build then stopped retrying and reported one attempt, zero successes, and one failure. Build `phase1a-self-hosted-pose-v5` therefore serves the required MediaPipe Pose JavaScript, graph, full model, and SIMD/non-SIMD WASM assets from the same GitHub Pages origin. The unused lite and heavy models are intentionally excluded. Other presentation dependencies remain CDN-hosted and are a separate offline-readiness limitation.
